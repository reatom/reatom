...
async function cleanupAutodocs() {
    ...
    try {
      ...
      try {
        ...
        if (remaining.length === 0) {
          ...
          console.log(`  ℹ Kept ${entry.name}/ directory (${remaining.length} files)`)
        }
      } catch {
      ...
      if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
        console.warn(`  ✗ Could not move ${entry.name}/README.md:`, (error as Error).message)
      }
    }
  }
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
      console.warn(`✗ Could not remove root README.md:`, (error as Error).message)
    }
  }

  const coreReadme = join(AUTODOCS_DIR, 'core', 'README.md')
  try {
    await unlink(coreReadme)
    console.log(`✓ Removed core/README.md`)
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
      console.warn(`✗ Could not remove core/README.md:`, (error as Error).message)
    }
  }
  ...
  for (const file of allFiles) {
    ...
    if (removed > 0) {
      ...
      console.log(`✓ Removed ${removed} underscore entries from ${relativePath}`)
      totalRemoved += removed
    }
  }
  
  if (totalRemoved > 0) {
    console.log(`\n✓ Total: ${totalRemoved} underscore-prefixed entries removed`)
  } else {
