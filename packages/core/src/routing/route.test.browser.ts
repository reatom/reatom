import { beforeEach, describe, expect, expectTypeOf, test, vi } from 'test'
import z from 'zod'
...
describe('codecs', () => {
  ...
  test('go() accepts Output types for params codec', () => {
    ...
    // go() should accept Output (number), not Input (string)
    expectTypeOf(route.go).parameter(0).toMatchObjectType<{ id: number }>()
    // route() returns Output
  ...
  test('go() accepts Output types for both params and search codecs', () => {
    ...
    // go() should accept Output types for both params and search
