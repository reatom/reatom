...
import { LL_NEXT, LL_PREV, reatomEnum, reatomLinkedList } from '@reatom/primitives';
...
describe('parseAtoms', () => {
	it('should return value', () => {
		...
		expectTypeOf(parseAtoms(ctx, Symbol.for('specialSymbol'))).toEqualTypeOf<symbol>()
	})

	it('should parse deep atoms', () => {
		const ctx = createTestCtx()
		expectTypeOf(parseAtoms(ctx, atom(() => atom('deep')))).toEqualTypeOf<string>()
		expectTypeOf(parseAtoms(ctx, atom(() => [atom(['deep'])]))).toEqualTypeOf<string[][]>()
	})

	it('should parse records', () => {
		const ctx = createTestCtx()
		expectTypeOf(parseAtoms(ctx, {
			someValue: atom(1),
			someDeep: {
				deep: {
					deep: atom('value'),
	...
	it('should parse deep structures', () => {
		const ctx = createTestCtx()
		expectTypeOf(parseAtoms(ctx, [[[[[atom('deepStruct')]]]]]))
