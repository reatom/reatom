...
const envs = z
  ...
  .parse({
    EXPO_PUBLIC_MAPLE_INGEST_ENDPOINT:
      process.env.EXPO_PUBLIC_MAPLE_INGEST_ENDPOINT,
    EXPO_PUBLIC_MAPLE_INGEST_PK_KEY:
      process.env.EXPO_PUBLIC_MAPLE_INGEST_PK_KEY,
  })
...
export const sendTrace = async (opts: SendTraceOptions) => {
  ...
  const res = await fetch(
    `${envs.EXPO_PUBLIC_MAPLE_INGEST_ENDPOINT}/v1/traces`,
    {
      method: 'POST',
      headers: {
