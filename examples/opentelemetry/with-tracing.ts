...
import { generateSpanId, generateTraceId, type ResourceAttributes, sendTrace, type SpanInput, type SpanKind } from './client'
import { serialize } from './serialize'

export const traceIdVar = variable<string>('tracing.traceId')
const spanIdVar = variable<string>('tracing.spanId')
const resourceAttributesVar = variable<ResourceAttributes>('tracing.resourceAttributes')
...
const startTracingFn: {
	<Params extends any[], Return>(
		cb: (...params: Params) => Return, ...params: Params
	): Return
	<Params extends any[], Return>(
...
export const startTracing = action(startTracingFn, '_startTracing') as GenericAction<typeof startTracingFn>

export const withStartTracing = withActionMiddleware(target => (next, ...params) => {
...
export const createTracingExtension = ({
	...
	const sendBatchesTraces = action(async () => {
		...
		try {
			...
			if (spans.length) {
				const resourceAttributes = typeof defaultResourceAttributes === 'function'
					? defaultResourceAttributes()
					: defaultResourceAttributes
	...
	return function withTracing(options: WithTracingOptions = {}) {
		...
		return (target: AtomLike) => {
			return target.extend(
				isAction(target) ? withActionMiddleware(() => (next, ...params) => {
					const startTime = Date.now()
					const { traceId, spanId, parentTraceId, parentSpanId } = startTracingInMiddleware()
					...
					try {
						...
						queueSpan({
							...
							parentSpanId,
							kind,
							status: { code: 'error', message: serialize(actionExecutionError) },
						})
					...
					try {
						...
						if (nextState instanceof Promise) {
							nextState
								...
								.catch(bind((error) => {
									const specialError = isAbort(error) || error instanceof Promise
									...
									queueSpan({
										...
										attributes: specialError ? {
											payload: isAbort(error) ? serialize(error) : '[Suspension]',
										} : undefined,
										status: specialError
											? { code: 'ok' }
											: { code: 'error', message: serialize(error) },
						...
						const { traceId, spanId, parentSpanId } = startTracingInMiddleware()
						...
						queueSpan({
							...
							parentSpanId,
							kind,
							status: { code: 'error', message: serialize(atomChangeError) },
						})
